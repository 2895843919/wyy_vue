<template>
  <div class="main">
    ...
    <div class="songplay_head">
      <span style="padding-right: 15px">{{ query_cat }}</span>
      <!-- 选择分类 -->
      <el-popover
      ref="popover"
      ...
      >
        ...
        <div
          style="display:flex;flex-direction:row;padding:20px:min-width:730px;width:730px"
...
export default {
  ...
  async created() {
    ...
    if (this.$route.query.cat == "全部" || this.$route.query.cat == "") {
    ...
    async handleCurrentChange(val) {
      ...
      if (this.page.currentPage >= this.page.tableData.length / 20) {
        ...
        for (let i = 0; i < page - 1; i++) {
          await top_playlist({
            cat: this.query_cat,
            offset: 20 * (this.page.currentPage - 1),
          })
            .then((res) => {
              res.playlists.forEach((item) => {
                item.coverImgUrl = item.coverImgUrl + "?param=130y130";
  ...
  watch: {
    "$route.query.cat"(to, from) {
      // location.reload()
      if (this.$route.query.cat != undefined || this.$route.query.cat!='全部') {
        // this.query_cat = this.$route.query.cat;
        // console.log(this.query_cat);
